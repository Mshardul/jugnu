...
**Origin:** raised while discussing [ticket 0025](tickets.md)'s addon dependency declaration. Explicitly not pursued yet. Shared code the user would not install alone is now a **helper** the shell downloads once ([ticket 0047](tickets.md) / vision rule 4) — that is not this idea. This idea is a live call from one running addon into another (e.g. MCP-style). Revisit only if a concrete addon design actually needs that; a simpler on-disk shared-state contract (declared paths, like `cleanup.paths` today) is the more likely shape if/when that happens.
