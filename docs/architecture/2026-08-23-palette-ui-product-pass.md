...
- **Physically separate Swift package** `shell/TestsExtended/` (not a tag on tests inside `JugnuCoreTests`, and not a second test target on `shell/Package.swift`). Default `cd shell && swift test` and CI run only the main package. Extended tests run with `make test-extended` (`cd shell/TestsExtended && swift test`). Nothing added to the extended package can start running just because a new test file forgot a tag.
...
- Default `swift test` in `shell/` (and CI) stays the unit suite. Live registry/install checks are a separate package: `make test-extended`.
